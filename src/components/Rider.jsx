...
import { SurfboardThruster } from "./SurfboardThruster";
...
export function Rider({
  ...
  return (
    <group {...props}>
      <group ref={hoverRef} scale={[1, 1, 1]}>
        {showDog && (
        ...
        {showVehicle && (
          <>
            {preview || vehicleGlb === "longboard" ? (
              <group name="Longboard" scale={[1.42, 1.42, 1.42]} rotation={[0, 0, 0]} position={[0, 0.87, 0]} visible={false}>
                <Vehicle model="longboard" />
              </group>
            ) : null}
            {preview || vehicleGlb === "skateboard" ? (
              <group name="Skateboard" scale={[7.8, 7.8, 7.8]} rotation={[0, Math.PI / 2, 0]} position={[0, 0.36, 0]} visible={false}>
                <Vehicle model="skateboard" />
              </group>
            ) : null}
            {preview || vehicleGlb === "surfboard_lucid_sn1" ? (
              <group name="Surfboard" ref={surfboardRef} scale={[1.7, 1.7, 1.7]} rotation={[2.7461615734037355, -0.476171473820599, 3.0479021033095606]} position={[0.04, 0.88, -2.33]}>
            ...
            {preview || vehicleGlb === "arcadia_longboard" ? (
              <group name="ArcadiaLongboard" scale={[0.0715, 0.0715, 0.0715]} rotation={[0, Math.PI / 2, 0]} position={[0.5, -34.692, -1.22]} visible={false}>
                <Vehicle model="arcadia_longboard" />
              </group>
            ) : null}
            {preview || vehicleGlb === "ufo" ? (
              <group name="UFO" ref={ufoRef} scale={[0.56, 0.56, 0.56]} rotation={[0, 0, 0]} position={[0.07, 0.3, 0.06]} visible={false}>
                <Vehicle model="ufo" />
                <group name="UFOThruster" position={[0, -0.2, 0]}>
                  <HoverThruster color="#39ff88" intensity={3} scale={1.1} sparks={14} />
                </group>
              </group>
            ) : null}
            {preview || vehicleGlb === "go-kart" ? (
              <group name="GoKart" scale={[2.48, 2.48, 2.48]} rotation={[0, 0, 0]} position={[0, 1.3, -0.7]} visible={false}>
                <Vehicle model="go-kart" />
                <group name="GoKartThrusters">
                  <HoverThruster color="#39ff88" intensity={3} scale={0.9} position={[0.35, 0, -0.7]} rotation={[Math.PI / 2, 0, 0]} sparks={14} />
                  <HoverThruster color="#39ff88" intensity={3} scale={0.9} position={[-0.35, 0, -0.7]} rotation={[Math.PI / 2, 0, 0]} sparks={14} />
                </group>
              </group>
            ) : null}
