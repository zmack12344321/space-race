...
/**
 * Standalone hover thruster tuned for the SURFBOARD. Fully independent of
 * <HoverThruster> — editing this file's logic never touches the UFO /
 * go-kart thrusters, and every prop below is exposed + ranged for Triplex.
 *
 * @typedef {Object} SurfboardThrusterProps
 * @property {string}  [color="#22e6ff"]      Tron edge/rim + beam tint (base when hueCycle is 0)
 * @property {string}  [coreColor="white"]     hot center of the nozzle (the "white light")
 * @property {number}  [scale=1.1]            overall size                       @min 0.1 @max 5
 * @property {number}  [intensity=3]          pointLight brightness              @min 0   @max 10
 * @property {number}  [beamTop=0.05]         beam radius at the mouth (top)   @min 0.01 @max 1
 * @property {number}  [beamBottom=0.1]       beam radius at the tail (bottom) @min 0.01 @max 1
 * @property {number}  [beamHeight=0.5]       beam length                      @min 0.05 @max 3
 * @property {number}  [rungs=4]              streaming energy rings            @min 0   @max 12
 * @property {number}  [flowSpeed=0.32]       ring/spark fall speed            @min 0   @max 3
 * @property {number}  [hueCycle=1.76]        hue rotate speed (0 = static)   @min 0   @max 5
 * @property {number}  [hueSat=1]             rainbow saturation               @min 0   @max 1
 * @property {number}  [hueLight=0.45]        rainbow lightness (lower=deeper) @min 0   @max 1
 * @property {number}  [sparks=10]            spark particles                  @min 0   @max 40
 * @property {boolean} [glow=true]             soft radial glow masks
 * @property {boolean} [fire=true]              flickering hot flare at the mouth
 */

// own glow texture (standalone — not shared with <HoverThruster>)
...
export const SurfboardThruster = ({
  color = "#22e6ff",
  coreColor = "white",
  scale = 1.1,
  intensity = 3,
  ...
  useFrame((state, delta) => {
    ...
    // manual billboard: face the camera with a single quaternion copy
