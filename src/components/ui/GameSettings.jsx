      ...
      <div className="mt-5 grid flex-1 gap-4 overflow-hidden lg:grid-cols-2">
        <div className="flex flex-col overflow-y-auto rounded-[1.5rem] border border-cyan-300/30 bg-white/[0.04] p-5 game-menu-scroll">
          ...
          <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {QUALITY_OPTONS.map((opt) => (
                  <button
                    key={opt.key}
                    type="button"
                    onClick={() => setSetting("quality", opt.key)}
            ...
            <SettingToggle label="Shadows" value={settings.shadows} onChange={(v) => setSetting("shadows", v)} />
            <SettingToggle label="Bloom" value={settings.bloom} onChange={(v) => setSetting("bloom", v)} />
            <SettingToggle label="Particles" value={settings.particles} onChange={(v) => setSetting("particles", v)} />
            <SettingToggle label="Screen Shake" value={settings.screenShake} onChange={(v) => setSetting("screenShake", v)} />
            <div className="pt-1">
              <button
                type="button"
                onClick={() => reset()}
