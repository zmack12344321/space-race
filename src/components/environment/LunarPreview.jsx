import { Billboard, useTexture, BakeShadows, Preload } from "@react-three/drei";
 ...
 */
export function LunarPreview(props) {
  const {
  ...
  const [lobbyFlatRadius] = useMultiplayerState("lobbyFlatRadius", 280);
  const [lobbyFlatFalloff] = useMultiplayerState("lobbyFlatFalloff", 120);

  // Spooky depth haze so the distant mountains melt into the night horizon.
  // (Fog setup is now handled by LunarEnvironment/LunarSky, but we can keep
  // this or remove it. LunarSky sets the same fog. We'll rely on LunarSky.)

  // Calculate the terrain height at the center so the podium/dog sit perfectly on the ground.
  const surfaceY = useMemo(() => getLunarHeight(0, 0, lunarSeed), [lunarSeed]);
  const lobbyHeightOptions = useMemo(
    () => ({
      flatZone: {
        center: { x: 0, z: 0 },
        radius: lobbyFlatRadius,
        innerRadius: Math.max(0, lobbyFlatRadius - lobbyFlatFalloff),
        height: surfaceY,
      },
    }),
    [surfaceY, lobbyFlatRadius, lobbyFlatFalloff]
  );
  ...
  return (
    <group>
      ...
      {/* Lunar ground — uses the exact same procedural generator as the real game map
          so it perfectly matches the vibe. Offset down by surfaceY so the dog is perfectly
          grounded at y=0. */}
      <group key={`ground-${lunarSeed}`} name="Ground" position={[0, -surfaceY, 0]}>
        <LunarTerrain isStatic staticRadius={8} seed={lunarSeed} heightOptions={lobbyHeightOptions} />
      </group>

      {/* Scattered lunar rocks — uses the exact same procedural scattering as the real game,
          but optimized for the static lobby map. */}
      <group key={`rocks-${lunarSeed}`} name="Rocks" position={[0, -surfaceY, 0]}>
        <LunarRocks isStatic staticRadius={8} seed={lunarSeed} clearRadius={lobbyFlatRadius} />
      </group>
