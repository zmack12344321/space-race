...
export function Rock({
  ...
  // Only poll distance when colliders are wanted — otherwise this is a cheap,
  // fully static decoration.
  useFrame(() => {
    if (!colliders || !groupRef.current) return;
    const d = camera.position.distanceTo(groupRef.current.getWorldPosition(_tmp));
    const isNear = d < PHYSICS_RADIUS;
    if (isNear !== near) setNear(isNear);
  });

  const body = colliders && near ? (
