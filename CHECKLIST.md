...
- [x] C7.4. `Rider.jsx` renders each vehicle as its OWN named literal <group> (Longboard / Skateboard / Surfboard / ArcadiaLongboard / UFO / GoKart) with literal position/rotation/scale props — so every vehicle is individually draggable/tunable in Triplex. `preview` still mounts all 6; production mounts only the selected one. NOTE: transforms must stay as literal JSX props (not pulled from a JS config) or Triplex loses edit access. `VEHICLE_TRANSFORMS` removed; single source of truth is now `Rider.jsx`.
...
- [ ] C7.6. In-game verify (user): in Triplex, open `Rider.jsx`, drag UFO / GoKart / boards so they sit under the dog. ufo/go-kart transforms are best-guesses (dog ~0.9 tall after 0.32 controller scale).
- [x] C7.7. Collision FIXED: `<CuboidCollider args={[0.45,0.45,0.8]} position={[0,0.45,0]}>` in `RiderController.jsx` — bottom now sits at the floor (y=0) instead of dipping to -0.1, so it no longer snags on the ground/ramps. Boards have NO colliders; the surfboard theory was wrong.
